...
/*   Updated: 2018/03/12 19:41:29 by qhonore          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "nm_otool.h"

static int	is_sort(t_env *e, struct nlist **tab, char *strtable, int i)
{
	if (e->sort == 'a'
	...
	e->magic))))
		return (0);
	else if (e->sort == 'N' && swap32(tab[i]->n_value, e->magic) >\
	swap32(tab[i + 1]->n_value, e->magic))
		return (0);
	return (1);
}
																	...
																	uint32_t i)
{
	...
	if (corrupted_ptr(e, strtable) || corrupted_ptr(e, array))
		return (0);
